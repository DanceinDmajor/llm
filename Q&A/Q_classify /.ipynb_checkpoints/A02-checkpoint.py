...
# 指定微调后模型的目录
finetuned_model_dir = 'output_sql_lora'

# 加载分词器
tokenizer = AutoTokenizer.from_pretrained(finetuned_model_dir, trust_remote_code=True)

# 加载微调后的模型
model = AutoModelForCausalLM.from_pretrained(finetuned_model_dir, device_map="cuda:0", trust_remote_code=True, bf16=True).eval()

# 设置生成配置
model.generation_config = GenerationConfig.from_pretrained(finetuned_model_dir, trust_remote_code=True, temperature=0.0001, top_p=1, do_sample=False, seed=1234)

new_question_file_dir = '/root/llm/processed_data/A01_question_classify.csv'
...
for cyc in range(len(company_file)):
    ...
    temp_index_cp = tokenizer(company_file[cyc:cyc+1]['公司称'][cyc])
    temp_index_cp = temp_index_cp['input_ids']
    company_index_list.append(temp_index_cp)
    
    
    
g = open('/root/llm/processed_data/A02_question_classify_entity.csv', 'w', newline='', encoding = 'utf-8-sig')
