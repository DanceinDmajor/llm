...
class DataArguments:
    data_paths: List[str] = field(
        default_factory=list, metadata={"help": "List of paths to the training data."}
    )
...
def make_supervised_data_module(
    ...
    # 合并多个数据集
    combined_data = []
    for data_path in data_args.data_paths:
        with open(data_path, "r") as f:
            combined_data.extend(json.load(f))

    train_dataset = dataset_cls(combined_data, tokenizer=tokenizer, max_len=max_len, system_message=system_message)
...
def train(data_paths, output_dir):
    global local_rank
...
def test_lora_model():
    ...
    prompt = "请查询在20210820日期，中信行业分类下钢铁一级行业中，当日收盘价波动最大（即最高价与最低价之差最大）的股票代码是什么？"
...
if __name__ == "__main__":
    ...
    # 合并两个数据集进行微调
    data_paths = ["/mnt/data/sql-lora-train.json", "/mnt/data/sql-lora-train-end.json"]
    train(data_paths=data_paths, output_dir="output_sql_lora")
